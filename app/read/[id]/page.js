...
export default function ReadPage() {
  ...
  // TTS state — which source is currently speaking
  const [speaking, setSpeaking] = useState(null) // 'book' | 'summary' | null
  ...
  function cleanForSpeech(text) {
    return text
      ...
      .replace(/[ \t]{2,}/g, ' ') // collapse extra whitespace
      .trim()
  }
  ...
  function speak(text, source) {
    ...
    utterance.rate = 0.92
    utterance.pitch = 1
    utterance.onstart = () => setSpeaking(source)
  ...
  async function handleSummarize() {
    ...
    try {
      const res = await fetch('/api/read', {
        ...
        body: JSON.stringify({
          text: book.content.slice(0, CHAR_LIMIT),
          mode: 'summarize',
        }),
      })
    ...
    <div className="min-h-screen bg-amber-50 flex flex-col">
      ...
      <header className="bg-white border-b border-stone-200 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link
            href="/dashboard"
            className="text-stone-400 hover:text-stone-700 transition-colors text-sm font-medium flex-shrink-0"
          >
            ← Library
          </Link>
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full px-6 py-8 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
          ...
          <div
            className={`px-6 py-4 font-mono text-xs text-stone-600 leading-relaxed whitespace-pre-wrap overflow-y-auto transition-all ${
              textExpanded ? 'max-h-[500px]' : 'max-h-36'
            }`}
          >
            {book?.content}
          </div>

          {/* Read Aloud button lives inside the text card */}
          <div className="px-6 pb-5">
            {speaking === 'book' ? (
              ...
              <button
                onClick={() => speak(book?.content, 'book')}
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-stone-400 mt-0.5">
                Sends first {Math.min(charCount, CHAR_LIMIT).toLocaleString()} of{' '}
                {charCount.toLocaleString()} characters to Claude
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-stone-800 hover:bg-stone-900 active:bg-black disabled:opacity-60 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2 text-sm"
          >
            {summarizing ? (
              <>
