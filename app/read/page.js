      ...
      <header className="bg-white border-b border-stone-200 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-4">
          ...
          <div className="ml-auto flex-shrink-0">
            <Link
              href="/signup"
              className="text-xs px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded-lg transition-colors"
            >
              Save to library →
            </Link>
          </div>
            ...
            <div className="mt-5 pt-5 border-t border-stone-100">
              <div className="flex items-center justify-between mb-3">
                ...
                {speaking === 'summary' ? (
                  ...
                  <button onClick={() => speak(stripMarkdown(summary), 'summary')}
              ...
              {speaking === 'summary' && (
                <div className="mt-3 flex items-center gap-2 text-xs text-amber-600">
                  <span className="flex gap-0.5 items-end">
                    {[0, 1, 2].map((i) => (
                      <span key={i} className="w-1 bg-amber-400 rounded-full animate-bounce"
                        style={{ height: '12px', animationDelay: `${i * 0.15}s` }} />
                    ))}
                  </span>
                  Reading aloud…
